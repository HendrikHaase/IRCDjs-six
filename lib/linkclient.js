...
function LinkClient(peer, serverName, options) {
    this.peer = peer;
...
LinkClient.prototype = {
    ...
    send: function () {
        ...
        try {
            ...
            console.log('LinkClient [' + this.name + '] ERROR: ' + exception);
            console.log('\t' + message);
        }
    },
    ...
    connected: function () {
        winston.info('Connected to remote server');
    },

    disconnected: function () {
        this.isConnected = false;
        winston.info('Disconnected from remote server');

        //  this.peer.log('LinkClient DISCONNECTED: ' + this.host + ':' + this.port);
    }
};
