...
Channel.prototype = {
    ...
    get isRegistered() {
        return this._modes.indexOf('r') > -1;
    },
    get isInviteOnly() {
    ...
    get names() {
        var channel = this;
        var rtn = this.users.map(function (user) {
    ...
    addModes: function (user, modes, arg) {
        ...
        if (targetUser && targetUser.isOper && !user.isService) {
            user.send(this.server.host, irc.errors.noPrivileges, user.nick, this.name, ":Permission Denied");
        } else {
    ...
    opModeAdd: function (mode, user, arg) {
        if (user.isOp(this) || user.isOper || user.isService) {
    ...
    addMode: {
        a: function (user, arg) {
            if (user.isService) {
                ...
                if (targetUser && !targetUser.isChannelOper(this) && !targetUser.isService) {
                    targetUser.operChan(this);
                    this.send(user.mask, 'MODE', this.name, '+a', targetUser.nick);
                }
            }
        },
        o: function (user, arg) {
    ...
    removeModes: function (user, modes, arg) {
        var channel = this;
        var targetUser = this.findUserNamed(arg);
        if (targetUser && targetUser.isOper) {
            user.send(this.server.host, irc.errors.noPrivileges, user.nick, this.name, ":Permission denied");
        } else {
            modes.slice(1).split('').forEach(function (mode) {
