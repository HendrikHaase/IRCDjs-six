...
Commands.prototype = {
    ...
    STATS: function (user, query, server) {
        ...
        var queryCommands = {
            ...
            M: function () {
                var commandKeys = Object.keys(Commands.prototype);
                var lastCmdNum = commandKeys.length - 1;
                // TODO: Stats history (how many times commands were used)
                for (i = 0, end = commandKeys.length; i < end; i++) {
                    var cmdKey = commandKeys[i];
                    user.send(self.server.host, irc.reply.statsN, user.nick, "M " + cmdKey + " 0");
                    if (i === lastCmdNum) {
